...
def make_train(config):
    ...
    meta_network = MetaDrift(config["LPO_HSIZE"])

    def train(meta_params, rng):
...
def eval_func(envs, meta_params, num_runs=8, iteration=0, hsize=128):
    pmap = jax.local_device_count() > 1

    returns_list = dict()
    runtimes = dict()
    meta_drift = MetaDrift(hsize)
    meta_params_pholder = meta_drift.init(jax.random.PRNGKey(0), jnp.zeros(8))
    ...
    params = jax.flatten_util.ravel_pytree(meta_params)[0]
    ...
    for i, env in enumerate(envs):
        ...
        all_configs[f"{env}"]["LPO_HSIZE"] = hsize
        ...
        wandb.log({f"{env}/return_lpo": fitness}, step=iteration)
        wandb.log({f"{env}/runtime_lpo": end - start}, step=iteration)
