...
import wandb
from algorithms.utils.architectures.open_ff import mlp_open_ff as optim
...
def make_train(config):
    ...
    @partial(jax.jit)
    def train(rng, meta_params=None):
        meta_opt = optim(hidden_size=config["OPTIM_HSIZE"])
...
def eval_func(envs, num_runs=8, iteration=0, title="", meta_params=None, hsize=16):
    ...
    meta_opt = optim(hidden_size=hsize)
