...
@gin.configurable
class recurrent_open_gru(opt_base.LearnedOptimizer):
    def __init__(
        self,
        exp_mult=0.001,
        step_mult=0.001,
        ...
        class _Opt(opt_base.Optimizer):
            ...
            def update(
                ...
                def _update_tensor(p, g, mom, k, dorm, carry, layer_prop, mask):
                    ...
                    return (update, gru_new_carry, update_)
                ...
                determ_update = [
                    updates_carry_leaves[i + 2]
                    for i in range(0, len(updates_carry_leaves), 3)
                ]

                def match_shapes_1(x, g):
                    x = jnp.expand_dims(x, -1)

                    if x.shape != g.shape:
                        x = jnp.tile(x, g.shape)

                    return x

                def match_shapes(x, g):
                    if x.shape != g.shape:
                        x = jnp.tile(x, [g.shape[0]] + [1])

                    return x

                lps = jax.tree_util.tree_map(match_shapes_1, layer_props, grad)
                masks = jax.tree_util.tree_map(match_shapes_1, mask, grad)
                dorms = jax.tree_util.tree_map(match_shapes, dormancies, grad)
                total_length = jax.flatten_util.ravel_pytree(grad)[0].shape[0]

                targets = {
                    "g": jax.flatten_util.ravel_pytree(grad)[0],
                    "mom": jax.flatten_util.ravel_pytree(rolling_features)[0].reshape(
                        (-1, 6)
                    ),
                    "k": jax.flatten_util.ravel_pytree(keys)[0].reshape((-1, 2)),
                    "dorm": jax.flatten_util.ravel_pytree(dorms)[0],
                    "carry": jax.flatten_util.ravel_pytree(full_carry)[0].reshape(
                        (-1, 8)
                    ),
                    "layer_props": jax.flatten_util.ravel_pytree(lps)[0],
                    "mask": jax.flatten_util.ravel_pytree(masks)[0],
                    "new_carry": jax.flatten_util.ravel_pytree(new_carry)[0].reshape(
                        (-1, 8)
                    ),
                    "updates": jax.flatten_util.ravel_pytree(updates)[0],
                    "determ_update": jax.flatten_util.ravel_pytree(determ_update)[0],
                    "param": jax.flatten_util.ravel_pytree(full_params)[0],
                    "training_prop": jnp.tile(training_prop, total_length),
                    "batch_prop": jnp.tile(batch_prop, total_length),
                }
